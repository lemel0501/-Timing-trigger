...
static void tgSendControlKeyboard(const String& chatId){
  ...
  // ← 修正 text 欄位
  String body = String("{\"chat_id\":\"")+chatId+"\",\"text\":\""+String(REPLY_MESSAGE)+"\",\"reply_markup\":"+kb+"}";
  String req  = "POST /bot"+cfg.token+"/sendMessage HTTP/1.1\r\nHost: api.telegram.org\r\nContent-Type: application/json\r\nContent-Length: "+String(body.length())+"\r\nConnection: close\r\n\r\n"+body;
  cli.print(req);

  // 預防舊鍵盤殘留：10秒後自動 remove_keyboard（若其實沒有鍵盤也不會出錯）
...
static void tgSendInlineOpen(const String& chatId){
  ...
  + "{\"text\":\"⚙️ 開啟設定 (WebApp)\",\"web_app\":{\"url\":\""
  + url
  + "\"}}]]}";


  String body = String("{\"chat_id\":\"")+chatId+"\",\"text\":\""+"\",\"reply_markup\":"+kb+"}";
  String req = "POST /bot" + cfg.token + "/sendMessage HTTP/1.1\r\nHost: api.telegram.org\r\nContent-Type: application/json\r\nContent-Length: " + String(body.length()) + "\r\nConnection: close\r\n\r\n" + body;
  cli.print(req);
}
...
void notifyOnline() {
  ...
  if (ok) {
    ...
    tgSendInlineOpen(cfg.chat);  // ← 只送底部 ③
...
// 產生附帶目前狀態的 WebApp URL（?cn0=..&ct1=HH:MM&chs=010101）
static String webappUrlWithState(){
  String url = WEBAPP_URL;            // 例如 https://lemel0501.github.io/nodemcu-notify/
  if (url.indexOf('?') < 0) url += "?"; else url += "&";

  // #1 門檻（達標推播）
  url += "cn0=" + String(cfg.cnt[0].target);

  // #2 每日時間（HH:MM）
  char hhmm[6]; snprintf(hhmm, sizeof(hhmm), "%02d:%02d", cfg.cnt[1].hh, cfg.cnt[1].mm);
  url += "&ct1=" + String(hhmm);

  // 六路開關目前狀態（1=ON,0=OFF），用 gTestActive 作為現況
  url += "&chs=";
...
if (up == "/PANEL" || txt == "⚙️ 開啟設定" || up == "PANEL") {
  // 1) 先找出本筆 update 的 chat.id（就在同一筆 JSON 範圍內）
  String chatId;
  int cpos = body.indexOf("\"chat\":{\"id\":", comma);
  if (cpos > 0 && cpos < scopeEnd) {
    int cstart = body.indexOf(":", cpos) + 1;
    int cend   = body.indexOf(",", cstart);
    if (cstart > 0 && cend > cstart) chatId = body.substring(cstart, cend);
    chatId.trim();
  }
  // 2) 沒抓到就退回 cfg.chat，抓到就用本筆 chatId 回覆
  String target = chatId.length() ? chatId : cfg.chat;
  if (target.length()) {
    tgSendInlineOpen(target);     // ★ 改這裡：回到同一個聊天室
  }
  pos = scopeEnd;
  continue;
}
