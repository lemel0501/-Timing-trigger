...
static long gLastInlineMsgId = 0;     // 記住上一顆 inline 按鈕的 message_id
...
static String webappUrlWithState();   // ← 加上這一行
// =========================【繼電器控制區】=========================
...
static unsigned long gKbHideAt = 0;  // 鍵盤收起的時間點
...
//在裝置端加一顆「⚙️ 開啟設定」鍵
#ifndef WEBAPP_URL
#define WEBAPP_URL "https://lemel0501.github.io/nodemcu-notify/"
#endif

// === Telegram 文字可自訂 ===
#ifndef REPLY_MESSAGE
#define REPLY_MESSAGE  "已送出設定鍵盤。"            // 若未使用 Reply 鍵盤，這行不會被看到
#endif
#ifndef INLINE_BUTTON_TEXT
#define INLINE_BUTTON_TEXT "⚙️ 開啟設定 (WebApp)"   // 按鈕上的文字（③）
#endif
#ifndef INLINE_MESSAGE
#define INLINE_MESSAGE "請點按下方按鈕開啟設定頁："   // ③ 上方提示文字
#endif
...
static void tgSendControlKeyboard(const String& chatId){
  const String url = webappUrlWithState();

  String kb = String("{\"keyboard\":[[")
            + "{\"text\":\"⚙️ 開啟設定\",\"web_app\":{\"url\":\"" + url + "\"}}]],"
  ...
  // 10 秒後收掉 Reply 鍵盤（就算沒鍵盤也無害）
  gKbHideAt = millis() + 10000UL;
}


// 刪除指定訊息（用來移除舊的 inline 按鈕，避免誤按舊連結）
static void tgDeleteMessage(const String& chatId, long msgId){
  if (msgId <= 0) return;
  WiFiClientSecure cli; cli.setInsecure();
  if (!cli.connect("api.telegram.org", 443)) return;

  String body = String("{\"chat_id\":\"")+chatId+"\",\"message_id\":"+String(msgId)+"}";
  String req  = "POST /bot"+cfg.token+"/deleteMessage HTTP/1.1\r\n"
                "Host: api.telegram.org\r\n"
                "Content-Type: application/json\r\n"
                "Content-Length: "+String(body.length())+"\r\n"
                "Connection: close\r\n\r\n"+body;
...
static void tgSendInlineOpen(const String& chatId){
  ...
  String kb = String("{\"inline_keyboard\":[[")
            + "{\"text\":\"" + String(INLINE_BUTTON_TEXT) + "\","
            +  "\"web_app\":{\"url\":\"" + url + "\"}}]]}";

  String body = String("{\"chat_id\":\"") + chatId +
                "\",\"text\":\"" + String(INLINE_MESSAGE) +
                "\",\"reply_markup\":" + kb + "}";
...
static void tgHideKeyboard(const String& chatId){
  ...
  const String body =
    ...
    "\",\"text\":\" \",\"reply_markup\":{\"remove_keyboard\":true}}";
...
// 回傳：從現在起到下一個 target HH:MM 還有幾秒（抓得到系統時間或 RTC 才會 >0）
static uint32_t secondsUntilNext(uint8_t targetH, uint8_t targetM){
  auto calc = [&](int h,int m,int s)->uint32_t{
    int cur = h*60 + m;
    int tar = targetH*60 + targetM;
    int dmin = (tar - cur + 24*60) % (24*60);
    if (dmin == 0) dmin = 24*60;        // 同一時間點視為「到下一天」
    long ds = (long)dmin*60 - s;
    if (ds < 1) ds = 1;
    return (uint32_t)ds;
  };
  ...
  // 1) 先用 NTP/系統時間
  struct tm t;
  if (getLocalTime(&t, 200)) return calc(t.tm_hour, t.tm_min, t.tm_sec);

  // 2) 退回 RTC
  if (gRtcReady){
    YqDateTime n = RTC.now();
    return calc(n.hour, n.minute, n.second);
  }

  // 3) 都拿不到時間就回 0（呼叫端會用既有 holdSec 當後備）
  return 0;
}
...
void notifyOnline() {
  ...
  if (ok) {
    ...
    //tgSendControlKeyboard(cfg.chat);//送出鍵盤通知
    //tgSendInlineOpen(cfg.chat);   // 只送底部 inline 按鈕（③）
  }
}
...
// 產生附帶目前狀態的 WebApp URL（本機優先，失敗再回 GitHub）
static String webappUrlWithState(){
  // 1) 先選 base：能取到本機 IP 就用本機頁，否則退回 GitHub
  String base;
  String ip = safeIP();                      // 取本機 / AP IP
  if (ip != "0.0.0.0") {
    base = "http://" + ip + "/";             // ← 本機頁（同源 HTTP）
  } else {
    base = WEBAPP_URL;                       // ← 退回 GitHub (HTTPS)
  }

  // 2) 帶上最新狀態 & 防快取參數（供 GitHub 版使用；本機版不依賴也無妨）
  String url = base;
  if (url.indexOf('?') < 0) url += "?"; else url += "&";

  url += "cn0=" + String(cfg.cnt[0].target);                             // #1門檻
  char hhmm[6]; snprintf(hhmm, sizeof(hhmm), "%02d:%02d", cfg.cnt[1].hh, cfg.cnt[1].mm);
  url += "&ct1=" + String(hhmm);                                         // #2時間

  url += "&chs=";                                                        // 6路現況
  for (int i=0;i<RELAY_COUNT;i++) url += (gTestActive[i] ? '1' : '0');

  url += "&ts=" + String((uint32_t)millis());                            // 防快取
  url += "&dev=http://" + safeIP();                                      // 告訴 WebApp 裝置位址
  return url;
}
...
// 將 WebApp 的 data(JSON) 套用到 cfg 後 save ＋ 推播差異摘要
static void applyWebAppConfig(const String& raw) {
  Serial.printf("[CFG] applyWebAppConfig: %d bytes\n", raw.length());

  AppConfig old = cfg;                 // ← 變更前快照
  String data = unwrapPayloadIfAny(raw);
  ...
  auto parseStrArray = [&](const String& key, String out[], int n){
    ...
    while (i<n && p<(int)arr.length()){
      ...
      out[i++] = arr.substring(s+1, e); p = e+1;
    }
    return (i>0);
  };
  auto parseIntArray = [&](const String& key, int out[], int n){
    ...
    while (i<n && p<(int)arr.length()){
      ...
      String tok = arr.substring(p, q); tok.trim();
      out[i++] = tok.toInt(); p = q+1;
    }
    return (i>0);
  };

  String t[RELAY_COUNT]; int hm[RELAY_COUNT]={0}; int hs[RELAY_COUNT]={0};
  ...
  if (hasT || hasHM || hasHS){
    for (int i=0;i<RELAY_COUNT;i++){
      if (hasT && t[i].length()>=4){
        int c = t[i].indexOf(':');
        int hh = constrain(t[i].substring(0,c).toInt(), 0, 23);
        int mm = constrain(t[i].substring(c+1).toInt(),   0, 59);
        cfg.sch[i].hh = hh; cfg.sch[i].mm = mm;
      }
      if (hasHM || hasHS){
        long hold = hasHM ? (long)hm[i]*60L : (long)cfg.sch[i].hold;
        if (hasHS) hold += (long)hs[i];
        if (hold < 0) hold = 0; if (hold > 3600) hold = 3600;
  ...
  // #2：每日時間（ct1）、訊息（cm1）
  {
    String ct1 = jsonGet(data, "ct1");
    if (ct1.length() && validHHMM(ct1)) {
      cfg.cnt[1].hh = ct1.substring(0,2).toInt();
      cfg.cnt[1].mm = ct1.substring(3,5).toInt();
    }
...
// ---- 六路開關狀態 chs:[0/1,...] ----
{
  int chs[RELAY_COUNT] = {0};
  if (parseIntArray("chs", chs, RELAY_COUNT)) {
    for (int i=0;i<RELAY_COUNT;i++) {
      if (chs[i]) {
        // ★ ON：不是用固定 hold 秒，而是跑到「該路設定的 HH:MM」為止
        uint32_t sec = secondsUntilNext(cfg.sch[i].hh, cfg.sch[i].mm);
        if (sec == 0) {                    // 拿不到時間 → 後備仍用原 hold
          sec = cfg.sch[i].hold;
        } else {
          // 可選：給個上限保險，避免被卡太久（例如 24h 封頂）
          if (sec > 24UL*3600) sec = 24UL*3600;
        }
        startRelayTimed(i, sec);
        uiShow("EXT CH"+String(i+1)+" 開始", "到 "+fmt2(cfg.sch[i].hh)+":"+fmt2(cfg.sch[i].mm));
        tgEnqueue("CH"+String(i+1)+" 輸出（計時開始");
      } else {
        // ★ OFF：外網關閉就立刻停止
        stopRelayIfActive(i, "WebApp");
      }
    }
  }
}
  ...
  // ---- 產生「變更摘要」並推播 ----
  auto hhmm = [](uint8_t H, uint8_t M){ char b[6]; snprintf(b,sizeof(b),"%02d:%02d",H,M); return String(b); };
  auto addChangeIf = [](std::vector<String>& diff, const String& title, const String& before, const String& after){
  ...
  // 星期
  if (old.wdMask != cfg.wdMask) changes.push_back("星期勾選變更（Mon..Sun bitmask）：" + String(old.wdMask) + " → " + String(cfg.wdMask));

  // 六路排程
  for (int i=0;i<RELAY_COUNT;i++){
    addChangeIf(changes, "CH"+String(i+1)+" 時間",      hhmm(old.sch[i].hh,old.sch[i].mm), hhmm(cfg.sch[i].hh,cfg.sch[i].mm));
    addChangeIf(changes, "CH"+String(i+1)+" 保持(秒)", String(old.sch[i].hold),           String(cfg.sch[i].hold));
  }

  // 計數器
  for (int i=0;i<2;i++){
    addChangeIf(changes, "計數器#"+String(i+1)+" 回報時間", hhmm(old.cnt[i].hh, old.cnt[i].mm), hhmm(cfg.cnt[i].hh, cfg.cnt[i].mm));
    addChangeIf(changes, "計數器#"+String(i+1)+" 訊息",      old.cnt[i].msg,                 cfg.cnt[i].msg);
    addChangeIf(changes, "計數器#"+String(i+1)+" 達標門檻",  String(old.cnt[i].target),       String(cfg.cnt[i].target));
  }

  String summary;
  if (!changes.empty()){
    summary = "⚙️ 設定已更新（" + String(changes.size()) + " 項）\n";
    for (size_t i=0;i<changes.size() && i<12;i++) summary += "• " + changes[i] + "\n";
    if (changes.size() > 12) summary += "…（其餘略）";
  } else {
    summary = "✅ 設定未變更";
  }
  tgEnqueue(summary);
    // ---- 立刻刷新 Telegram 鍵盤（用最新參數重組 URL）----
    if (cfg.chat.length()) tgHideKeyboard(cfg.chat);     // 可選：先收起舊鍵盤
    //if (cfg.chat.length()) tgSendControlKeyboard(cfg.chat); // 重送含新 URL 的鍵盤
}
  ...
  static void tgUpdatePollLoop(){
    static unsigned long last = 0;
        // ★ 在每次輪詢時順便檢查是否到點要收掉 Reply 鍵盤
// 檢查是否到達收鍵盤時間
if (gKbHideAt && millis() >= gKbHideAt) {
  gKbHideAt = 0;
...
// ---- 取得本筆 update 的文字內容（如 /k、/check_token）----
...
if (tpos > 0 && tpos < scopeEnd) {
  ...
  String up = txt; up.trim(); up.toUpperCase();

  if (up == "/K" || up == "K") {
    if (cfg.chat.length()) {
      gKbHideAt = 0;                   // 先清掉舊的倒數，避免同輪被收掉
      tgSendControlKeyboard(cfg.chat); // 送出 Reply 鍵盤
      gKbHideAt = millis() + 10000UL;  // 明確設為 10 秒
    }
    pos = scopeEnd;
    continue;
  }
  
    // /F 或 F → 立刻收折鍵盤（送空白鍵盤），並清掉倒數
    if (up == "/F" || up == "F") {
      if (cfg.chat.length()) {
        tgHideKeyboard(cfg.chat);          // remove_keyboard=true
        gKbHideAt = 0;                     // 取消倒數，避免重複觸發
      }
      pos = scopeEnd;
      continue;
    }
  ...
  for (int i = 0; i < RELAY_COUNT; ++i) {
    ...
    if (curH == cfg.sch[i].hh && curM == cfg.sch[i].mm) {
      if (lastTrigKey[i] != curKey) {  // 同分鐘不重複
        ...
        if (gTestActive[i]) {
          // ★ 若這一刻本來就因外網控制而處於 ON，別再啟動以免延長
          Serial.printf("[SCH] CH%d already active at schedule time, skip start\n", i+1);
        } else {
...
static inline void addCorsHeaders() {
  ...
  srv.sendHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS");  // ← 加上 GET
...
void setup() {
  ...
  // 允許預檢
srv.on("/webapp-export", HTTP_OPTIONS, [](){
  addCorsHeaders();
  srv.send(200, "text/plain", "");
});

// 回傳目前最新設定（給 WebApp 覆蓋畫面）
srv.on("/webapp-export", HTTP_GET, [](){
  addCorsHeaders();
  char hhmm[6]; snprintf(hhmm, sizeof(hhmm), "%02d:%02d", cfg.cnt[1].hh, cfg.cnt[1].mm);
  String chs; for (int i=0;i<RELAY_COUNT;i++) chs += (gTestActive[i]?'1':'0');
  String j = String("{\"cn0\":") + cfg.cnt[0].target +
             ",\"ct1\":\"" + String(hhmm) + "\"" +
             ",\"chs\":\"" + chs + "\"}";
...
srv.on("/webapp-save", HTTP_POST, [](){
  ...
  srv.on("/k", HTTP_GET, [](){
    ...
    // 先確保把任何殘留的 Reply 鍵盤關掉（就算沒有也不會出錯）
    tgHideKeyboard(cfg.chat);
  
    // 只送一顆「內嵌開啟」按鈕（③）
    tgSendControlKeyboard(cfg.chat);
  
    // 再保險：10 秒後再送一次 remove_keyboard，模擬「默默收折」
    gKbHideAt = millis() + 10000UL;
